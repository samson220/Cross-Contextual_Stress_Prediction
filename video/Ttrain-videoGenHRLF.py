...
for clf_name, (clf, param_grid) in classifiers.items():
    ...
    clf_random_search = RandomizedSearchCV(clf, param_distributions=param_grid, n_iter=1, cv=kf, scoring='f1_macro', random_state=42, n_jobs=-1)
    clf_random_search.fit(X_train, y_train)

    # Find the best score and fold for the current classifier
    for i in range(10):
        fold_score = clf_random_search.cv_results_[f'split{i}_test_score'][clf_random_search.best_index_]
...
# Print the best classifier, fold, and its score
...
with open(f'best_classifier_{best_classifier_name}.pkl', 'wb') as f:
    pickle.dump(best_classifier, f)

# Make predictions using the best classifier
best_classifier.fit(X_train, y_train)  # Refit the best classifier
