...
# # Convert time columns to datetime objects and then to ordinal format
# time_cols = ['Time_Biotrace', 'Time_Videorating', 'Time_Light', 'Time_Accel', 'Time_GPS']
# for col in time_cols:
#     data[col] = pd.to_datetime(data[col], format='%H:%M:%S:%f').apply(lambda x: x.toordinal())
...
# # Selecting numerical columns for scaling (excluding the target variable)
# numerical_cols = data.select_dtypes(include=[np.number]).columns.tolist()
# numerical_cols.remove('Rating_Videorating')
...
# # Ensuring all columns are in the correct format (if needed)
# # ... (Not performed here as the dataset seems to have appropriate data types)
...
for clf_name, (clf, param_grid) in classifiers.items():
    ...
    save_obj(clf_results, f'/Users/samson/Documents/Final2/video/results0/{clf_name}_F2train_video_RS-10kf')
